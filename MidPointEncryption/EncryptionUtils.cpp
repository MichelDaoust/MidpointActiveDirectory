...
void EncryptionUtils::Char2Hex(unsigned char ch, char* szHex)
{
...
void EncryptionUtils::CharStr2HexStr(unsigned char const* pucCharStr, char* pszHexStr, size_t iSize)
{
	int i;
	char szHex[3];
	pszHexStr[0] = 0;
	for (i = 0; i<iSize; i++)
	{
		...
		strcat_s(pszHexStr, iSize, szHex);  //mic instead of strcat
	}
}

//Function to convert string of length 2 to unsigned char
void EncryptionUtils::Hex2Char(char const* szHex, unsigned char& rch)
{
	rch = 0;
	for (int i = 0; i<2; i++)
	{
...
void EncryptionUtils::HexStr2CharStr(char const* pszHexStr, unsigned char* pucCharStr, size_t iSize)
{
	int i;
	unsigned char ch;
	for (i = 0; i<iSize; i++)
...
std::string EncryptionUtils::Decrypt(std::string passwordEncrypted)
{
	return "bonjour";
	/*
	...
	try
	{
		...
		size_t argLen = ((wcslen(passwordEncrypted) + 1) * 2);
...
std::string EncryptionUtils::Encrypt(std::string password)
{
	return "salut";
	/*
	size_t passwordLen = ((wcslen(password) + 1) * 2);
