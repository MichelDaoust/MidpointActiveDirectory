...
void setLogFilePermissions(wchar_t *logFile)
{
	...
	hFile = CreateFile(logFile, FILE_APPEND_DATA, FILE_SHARE_WRITE, &sa, CREATE_NEW, FILE_ATTRIBUTE_NORMAL, 0);
			...
			if (pwdChange)
			{
				...
				if (ret_val == RPC_S_OK)
				{
					...
					else
					{
						...
						totalpath = NULL;
						wchar_t message[] = L"Error couldn't convert the GUID to string";
